...
- **Fuzzer expanded — three new targets covering the native Clojure surface.** The post-Stream-B work (`meme.tools.clj.parser.*`, `clj-lang.api`, AST-tier roundtrip) had zero coverage-guided exploration; only fixed-corpus vendor cross-check stood between regressions and a green build. New targets:
  ...
  - **`forms-to-clj` reader + opt-passing fixes.** The differential `read-clj-all` helper now binds `*default-data-reader-fn* tagged-literal` so unknown tags become `TaggedLiteral` instead of throwing (matched meme's tagged-literal handling and `clojure.core/read-string` under the same binding). The target also now passes `:resolve-keyword tu/cross-check-resolve-keyword` to `m1clj->forms` and binds `*ns* user` for `expand-forms` and the differential reader so deferred `CljAutoKeyword` records resolve eagerly — without this, `::local` stayed a record on the LHS but became `:user/local` on the RHS, producing a false positive.
  - **`normalize-form` gaps closed.** Cross-check's first real run surfaced two missing rules: `##NaN != ##NaN` (IEEE 754 — collapsed to a sentinel) and the `#()` rest-arg `%&` (read-string emits `rest__<id>#`, native emits `%&` — collapsed to `<rest>`). Both are fuzzer-found bugs in the cross-check normalisation, not in the production parsers; vendor cross-check still passes on all 7 projects.
...
- **CLJS test suite is now green** (was 313 errors, 2 failures). One root cause: `(instance? meme.tools.clj.ast.nodes.CljSymbol x)` (dotted JVM FQN) doesn't resolve on ClojureScript — defrecord types are JS exports referenced by the namespace path, not by the dotted form. CLJS produced `undefined` and `instanceof` threw `"Right-hand side of 'instanceof' is not an object"` 312 times across formatter/printer/roundtrip/api tests. Fixed in four files (`m1clj-lang.printer`, `m1clj-lang.form-shape`, `meme.tools.clj.ast.lower`, `clj-lang.api`) by adding `:refer` for record types on CLJS plus the existing JVM `:import`, then using bare names at every call site. The remaining error (`atoms-equiv`) was a test-side oversight: the loop included `"0x2A"` but hex literals are JVM-only — gated to `#?(:clj …)` alongside `42N` / `1.5M` / `1/2`.
