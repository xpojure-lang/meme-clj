(ns beme.core-test
  "Tests for the beme.core public API."
...
(deftest clj->forms-eof-sentinel-no-collision
  (testing ":beme.core/eof as top-level form does not truncate output"
    (is (= [:beme.core/eof '(def y 2)]
           (core/clj->forms ":beme.core/eof (def y 2)"))))
  (testing ":beme.core/eof inside a form is preserved"
    (is (= '[(def x :beme.core/eof)]
           (core/clj->forms "(def x :beme.core/eof)"))))))
