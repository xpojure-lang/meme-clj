...
(defn- tmp-meme
  "Write content to a temp .m1clj file, return the File."
  [content]
  (let [f (File/createTempFile "m1clj-e2e-" ".m1clj")]
    (.deleteOnExit f)
    (spit f content)
    f))
...
(defn- tmp-clj
  ...
  [content]
  (let [f (File/createTempFile "m1clj-e2e-" ".clj")]
    (.deleteOnExit f)
    (spit f content)
    f))
...
(defn- tmp-ext
  ...
  [ext content]
  (let [f (File/createTempFile "m1clj-e2e-" (str "." ext))]
    (.deleteOnExit f)
    (spit f content)
    f))
...
(deftest help-test
  (let [{:keys [out exit]} (bb-meme)]
    ...
    (is (str/includes? out "meme to-m1clj"))
...
(deftest inspect-test
  (let [{:keys [out exit]} (bb-meme "inspect")]
    ...
    (is (str/includes? out "to-m1clj"))))

(deftest inspect-lang-test
  (let [{:keys [out exit]} (bb-meme "inspect" "--lang" "m1clj")]
    (is (zero? exit))
    (is (str/includes? out "Lang: m1clj"))))
...
(deftest to-clj-file-test
  (let [f (tmp-meme "defn(foo [x] +(x 1))")
        clj-path (str/replace (str f) #"\.m1clj$" ".clj")
...
(deftest to-clj-lang-test
  (let [f (tmp-meme "f(x y)")
        {:keys [out exit]} (bb-meme "to-clj" (str f) "--stdout" "--lang" "m1clj")]
    (is (zero? exit))
    (is (= "(f x y)\n" out))))
...
(deftest to-m1clj-stdout-test
  (let [f (tmp-clj "(defn foo [x] (+ x 1))")
        {:keys [out exit]} (bb-meme "to-m1clj" (str f) "--stdout")]
    (is (zero? exit))
    (is (= "defn(foo [x] +(x 1))\n" out))))

(deftest from-clj-is-alias-of-to-m1clj
  (let [f (tmp-clj "(defn foo [x] (+ x 1))")
        {:keys [out exit]} (bb-meme "from-clj" (str f) "--stdout")]
    (is (zero? exit))
    (is (= "defn(foo [x] +(x 1))\n" out))))

(deftest to-m1clj-file-test
  (let [f (tmp-clj "(defn foo [x] (+ x 1))")
        meme-path (str/replace (str f) #"\.clj$" ".m1clj")
        meme-file (io/file meme-path)]
    (try
      (let [{:keys [exit]} (bb-meme "to-m1clj" (str f))]
        (is (zero? exit))
        (is (.exists meme-file))
        (is (= "defn(foo [x] +(x 1))\n" (slurp meme-file))))
      (finally (.delete meme-file)))))

(deftest to-m1clj-cljc-file-test
  (let [f (tmp-ext "cljc" "(defn foo [x] (+ x 1))")
        meme-path (str/replace (str f) #"\.cljc$" ".m1clj")
        meme-file (io/file meme-path)
        original-content (slurp f)]
    (try
      (let [{:keys [exit]} (bb-meme "to-m1clj" (str f))]
        (is (zero? exit))
        (is (.exists meme-file) ".m1clj output file should be created")
        (is (= "defn(foo [x] +(x 1))\n" (slurp meme-file)))
        ;; Critical: original .cljc must NOT be overwritten
        (is (= original-content (slurp f)) ".cljc source must not be overwritten"))
      (finally (.delete meme-file)))))

(deftest to-m1clj-cljs-file-test
  (let [f (tmp-ext "cljs" "(defn foo [x] (+ x 1))")
        meme-path (str/replace (str f) #"\.cljs$" ".m1clj")
        meme-file (io/file meme-path)
        original-content (slurp f)]
    (try
      (let [{:keys [exit]} (bb-meme "to-m1clj" (str f))]
...
(deftest format-lang-test
  (let [f (tmp-meme "defn(foo [x] +(x 1))")
        {:keys [out exit]} (bb-meme "format" (str f) "--stdout" "--lang" "m1clj")]
    (is (zero? exit))
    (is (= "defn( foo [x] +(x 1))\n" out))))

;; ---------------------------------------------------------------------------
;; Scar tissue: Bug #3 — Babashka warns about .m1clj require limitation
...
;; ---------------------------------------------------------------------------
;; Scar tissue: Bug #4 — bb meme run with nested .m1clj require
;; The CLI run command now installs the loader. On Babashka, nested .m1clj
...
(deftest compile-to-custom-dir
  (let [f (tmp-meme "defn(foo [x] +(x 1))")
        out-dir (str (System/getProperty "java.io.tmpdir") "/m1clj-e2e-compile-" (System/nanoTime))]
    (try
      (let [{:keys [out exit]} (bb-meme "compile" (str f) "--out" out-dir)]
        ...
        (let [clj-name (str/replace (.getName f) #"\.m1clj$" ".clj")
...
(deftest compile-preserves-relative-paths
  (let [dir (io/file (System/getProperty "java.io.tmpdir")
                     (str "m1clj-e2e-src-" (System/nanoTime)))
        sub (io/file dir "my_ns")
        out-dir (str dir "-out")]
    (try
      (.mkdirs sub)
      (spit (io/file sub "core.m1clj") "ns(my-ns.core)\ndef(x 42)\n")
...
(deftest transpile-is-canonical-name-for-compile
  (let [f (tmp-meme "defn(foo [x] +(x 1))")
        out-dir (str (System/getProperty "java.io.tmpdir") "/m1clj-e2e-tx-" (System/nanoTime))]
    (try
...
(deftest build-produces-jvm-bytecode
  (let [src-dir (io/file (System/getProperty "java.io.tmpdir")
                         (str "m1clj-e2e-build-" (System/nanoTime)))
        demo (io/file src-dir "demo")
        aot-dir (str src-dir "-aot")]
    (try
      (.mkdirs demo)
      (spit (io/file demo "core.m1clj") "ns(demo.core)\ndefn(greet [x] str(\"hello \" x))")
...
(deftest compile-reports-errors
  (let [dir (io/file (System/getProperty "java.io.tmpdir")
                     (str "m1clj-e2e-err-" (System/nanoTime)))
        out-dir (str dir "-out")]
    (try
      (.mkdirs dir)
      (spit (io/file dir "good.m1clj") "def(x 1)")
      (spit (io/file dir "bad.m1clj") "def(x")
...
;; ---------------------------------------------------------------------------
;; load-file of .m1clj via bb meme run
;; ---------------------------------------------------------------------------
...
(deftest run-load-file-meme
  (let [lib (tmp-meme "defn(greet [x] str(\"hi \" x))")
    ...
    (is (zero? exit))
    (is (= "hi bb\n" out)
        "load-file of .m1clj should work from within bb meme run")))

;; ---------------------------------------------------------------------------
;; JVM require of .m1clj namespace (loader auto-install via language tier)
;;
;; Demonstrates that the loader is a core DX utility installed by
;; m1clj-lang.run automatically — the CLI does no manual install, yet
;; require of a .m1clj file on the classpath resolves through the meme
;; pipeline. If m1clj-lang.run/run-string ever stops auto-installing the
;; loader, this test fails.
;; ---------------------------------------------------------------------------

(deftest run-jvm-require-meme-namespace
  (let [dir (io/file (System/getProperty "java.io.tmpdir")
                     (str "m1clj-e2e-req-" (System/nanoTime)))
        lib-dir (io/file dir "mylib")]
    (try
      (.mkdirs lib-dir)
      (spit (io/file lib-dir "core.m1clj")
            "ns(mylib.core)\ndefn(greet [x] str(\"hi \" x))\n")
      (let [main (io/file dir "main.m1clj")]
        ...
        (let [{:keys [out err exit]}
          ...
          (is (str/includes? out "hi jvm")
              "require of .m1clj namespace from classpath should resolve through the loader")))
      (finally
...
(deftest run-command-line-args-not-leaked
  (let [f (tmp-meme "println(pr-str(*command-line-args*))")
    ...
    (is (not (str/includes? out ".m1clj")) "filename not leaked")))
