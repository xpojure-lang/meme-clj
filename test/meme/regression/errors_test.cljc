...
#?(:clj
   (deftest read-string-errors-include-location
     ...
     (testing "malformed regex includes location"
       (let [e (try (lang/m1clj->forms "#\"[unclosed\"")
                    nil
                    (catch Exception e e))]
         (is (some? e))
         (is (= 1 (:line (ex-data e))))
...
(deftest namespaced-map-errors-include-location
  (testing "malformed namespaced map has :line/:col in ex-data"
    (let [ex (try (lang/m1clj->forms "#:ns{:a}")
...
#?(:clj
   (deftest read-eval-blocked-in-opaque-forms
     (testing "#=() blocked in syntax-quote"
       (is (thrown? Exception (lang/m1clj->forms "`(#=(+ 1 2))"))))
     (testing "#=() blocked in namespaced map"
       (is (thrown? Exception (lang/m1clj->forms "#:ns{:k #=(+ 1 2)}"))))
     (testing "#=() blocked in reader conditional"
       (is (thrown? Exception (lang/m1clj->forms "#?(:clj #=(+ 1 2))"))))))
...
#?(:cljs
   (deftest cljs-unsupported-number-formats
     (testing "BigInt N suffix"
       (is (thrown-with-msg? js/Error #"BigInt" (lang/m1clj->forms "42N"))))
     (testing "BigDecimal M suffix"
       (is (thrown-with-msg? js/Error #"BigDecimal" (lang/m1clj->forms "42M"))))
     (testing "Ratio"
       (is (thrown-with-msg? js/Error #"Ratio" (lang/m1clj->forms "1/2"))))
     (testing "Hex"
       (is (thrown-with-msg? js/Error #"Hex" (lang/m1clj->forms "0xFF"))))
     (testing "Radix"
       (is (thrown-with-msg? js/Error #"Radix" (lang/m1clj->forms "2r1010"))))
     (testing "Octal"
       (is (thrown-with-msg? js/Error #"Octal" (lang/m1clj->forms "010"))))))
...
(deftest unclosed-reader-conditional-has-context
  (testing "unclosed #?( is :incomplete"
    (let [e (try (lang/m1clj->forms "#?(:clj 1")
                 nil
...
(deftest surrogate-unicode-escape-accepted
  (testing "\\uD800 (surrogate) is accepted to match Clojure"
    (is (some? (lang/m1clj->forms "\"\\uD800\""))))
  (testing "\\uDFFF (surrogate) is accepted to match Clojure"
    (is (some? (lang/m1clj->forms "\"\\uDFFF\""))))
  (testing "valid unicode escapes still work"
    (is (some? (lang/m1clj->forms "\"\\u0041\"")))
    (is (some? (lang/m1clj->forms "\"\\uFFFF\"")))))
