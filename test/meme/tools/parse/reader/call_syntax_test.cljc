(ns meme.tools.parse.reader.call-syntax-test
...
(deftest symbol-head-arity-1
  (is (= '[(foo x)] (lang/meme->forms "foo(x)"))))

(deftest symbol-head-arity-2
  (is (= '[(foo x y)] (lang/meme->forms "foo(x y)"))))

(deftest symbol-head-arity-3+
  (is (= '[(foo a b c d)] (lang/meme->forms "foo(a b c d)"))))

(deftest symbol-head-operator
  (is (= '[(+ 1 2 3)] (lang/meme->forms "+(1 2 3)"))))

(deftest symbol-head-nested
  (is (= '[(a (b (c x)))] (lang/meme->forms "a(b(c(x)))"))))
...
(deftest keyword-head-arity-0
  (is (= '(:foo) (first (lang/meme->forms ":foo()")))))

(deftest keyword-head-arity-1
  (is (= '(:foo x) (first (lang/meme->forms ":foo(x)")))))

(deftest keyword-head-arity-2
  (is (= '(:foo x y) (first (lang/meme->forms ":foo(x y)")))))

(deftest keyword-head-arity-3+
  (is (= '(:foo a b c d) (first (lang/meme->forms ":foo(a b c d)")))))

(deftest keyword-head-namespaced
  (is (= '(:ns/key x) (first (lang/meme->forms ":ns/key(x)")))))

(deftest keyword-head-require-pattern
  (is (= '(:require [bar]) (first (lang/meme->forms ":require([bar])")))))

(deftest keyword-head-import-pattern
  (is (= '(:import [java.util Date]) (first (lang/meme->forms ":import([java.util Date])")))))
...
(deftest vector-head-arity-0
  (is (= '([x]) (first (lang/meme->forms "[x]()")))))

(deftest vector-head-arity-1
  (is (= '([x] 1) (first (lang/meme->forms "[x](1)")))))

(deftest vector-head-arity-2
  (is (= '([x] (+ x 1) x) (first (lang/meme->forms "[x](+(x 1) x)")))))

(deftest vector-head-multi-params
  (is (= '([x y] (+ x y)) (first (lang/meme->forms "[x y](+(x y))")))))

(deftest vector-head-empty-vector
  (is (= '([] 42) (first (lang/meme->forms "[](42)")))))
...
(deftest set-head-arity-1
  (let [form (first (lang/meme->forms "#{:a :b}(x)"))]
    (is (set? (first form)))
    (is (= 'x (second form)))))
...
(deftest map-head-arity-1
  (let [form (first (lang/meme->forms "{:a 1}(:a)"))]
    (is (map? (first form)))
    (is (= :a (second form)))))

(deftest map-head-arity-0
  (let [form (first (lang/meme->forms "{:a 1}()"))]
    (is (map? (first form)))
    (is (= 1 (count form)))))
...
(deftest keyword-spacing-adjacent
  (is (= '(:k x) (first (lang/meme->forms ":k(x)")))))
...
(deftest set-spacing-adjacent
  (let [form (first (lang/meme->forms "#{:a}(x)"))]
    (is (= 'x (second form)))))
...
(deftest pattern-defn-single-arity
  (is (= '[(defn greet [name] (str "Hello " name))]
         (lang/meme->forms "defn(greet [name] str(\"Hello \" name))"))))

(deftest pattern-defn-multi-arity
  (let [result (first (lang/meme->forms "defn(foo [x](x) [x y](+(x y)))"))]
...
(deftest pattern-ns-require
  (let [result (first (lang/meme->forms "ns(my.app :require([clojure.string :as str]))"))]
    (is (= 'ns (first result)))
    (is (= 'my.app (second result)))
    (is (= :require (first (nth result 2))))))

(deftest pattern-ns-require-import
  (let [result (first (lang/meme->forms "ns(my.app :require([clojure.string :as str]) :import([java.util Date]))"))]
    (is (= 4 (count result)))
    (is (= :require (first (nth result 2))))
...
(deftest pattern-defrecord-with-impls
  (let [result (first (lang/meme->forms "defrecord(Circle [r] Shape area([this] *(Math/PI r r)))"))]
...
(deftest pattern-let-with-calls
  (is (= '[(let [x (+ 1 2)] (* x x))]
         (lang/meme->forms "let([x +(1 2)] *(x x))"))))

(deftest pattern-threading
  (is (= '[(-> x (foo 1) (bar 2))]
         (lang/meme->forms "->(x foo(1) bar(2))"))))

(deftest pattern-try-catch
  (is (= '[(try (risky) (catch Exception e (handle e)))]
         (lang/meme->forms "try(risky() catch(Exception e handle(e)))"))))
