(ns meme.tools.parse.reader.calls-test
...
(deftest parse-do-call
  (let [result (first (lang/meme->forms "do(println(\"a\") println(\"b\"))"))]
    (is (= 'do (first result)))
    (is (= 3 (count result)))))
...
(deftest parse-def-with-expression
  (is (= '[(def x (+ 1 2))]
         (lang/meme->forms "def(x +(1 2))"))))

(deftest parse-defn-single
  (let [result (first (lang/meme->forms "defn(greet [name] println(str(\"Hello \" name)))"))]
    (is (= 'defn (first result)))
    (is (= 'greet (second result)))
    (is (= '[name] (nth result 2)))))

(deftest parse-defn-with-multi-body
  (let [result (first (lang/meme->forms "defn(greet [name] println(\"hi\") :ok)"))]
    (is (= 'defn (first result)))
    (is (= 'greet (second result)))
    (is (= '[name] (nth result 2)))
...
(deftest parse-defn-docstring
  (let [result (first (lang/meme->forms "defn(greet \"Greets a person\" [name] println(str(\"Hello \" name)))"))]
    (is (= "Greets a person" (nth result 2)))
    (is (= '[name] (nth result 3)))))

(deftest parse-defn-multi-arity
  ;; Vector-as-head: [args](body) → ([args] body)
  (let [result (first (lang/meme->forms
...
(deftest parse-defn-multi-body-forms
  (let [result (first (lang/meme->forms "defn(greet [name] println(str(\"Hello \" name)) name)"))]
    (is (= 'defn (first result)))
    (is (= 5 (count result)))
    (is (= 'name (nth result 4)))))

(deftest parse-defn-private
  (let [result (first (lang/meme->forms "defn-(helper [x] +(x 1))"))]
    (is (= 'defn- (first result)))
    (is (= 'helper (second result)))))

(deftest parse-defmacro
  (let [result (first (lang/meme->forms "defmacro(when-let [bindings body] list(if bindings body nil))"))]
...
(deftest parse-fn-simple
  (let [result (first (lang/meme->forms "fn([x] +(x 1))"))]
    (is (= 'fn (first result)))
    (is (= '[x] (second result)))
    (is (= '(+ x 1) (nth result 2)))))

(deftest parse-fn-two-args
  (let [result (first (lang/meme->forms "fn([x y] *(x y))"))]
    (is (= 'fn (first result)))
    (is (= '[x y] (second result)))))

(deftest parse-fn-with-multi-body
  (let [result (first (lang/meme->forms "fn([acc item] assoc(acc :id(item) :balance(item)))"))]
    (is (= 'fn (first result)))
...
(deftest parse-let
  (let [result (first (lang/meme->forms "let([x 1, y +(x 1)] *(x y))"))]
    (is (= 'let (first result)))
    (is (= '[x 1 y (+ x 1)] (second result)))))

(deftest parse-let-with-destructuring
  (let [result (first (lang/meme->forms "let([{:keys [id name]} person] println(id name))"))]
    (is (= 'let (first result)))
...
(deftest parse-loop
  (let [result (first (lang/meme->forms "loop([i 0, acc []] if(>=(i 10) acc recur(inc(i) conj(acc i))))"))]
...
(deftest parse-for
  (let [result (first (lang/meme->forms "for([x xs, y ys, :when >(x y)] [x y])"))]
    (is (= 'for (first result)))
    (let [bindings (second result)]
      (is (= 'x (nth bindings 0)))
...
(deftest parse-doseq
  (let [result (first (lang/meme->forms "doseq([x xs] println(x))"))]
...
(deftest parse-if-call
  (is (= '[(if (> x 0) "positive" "negative")]
         (lang/meme->forms "if(>(x 0) \"positive\" \"negative\")"))))

(deftest parse-if-single-branch
  (is (= '[(if (> x 0) "positive")]
         (lang/meme->forms "if(>(x 0) \"positive\")"))))

(deftest parse-when-call
  (let [result (first (lang/meme->forms "when(>(x 0) println(\"positive\") do-something(x))"))]
...
(deftest parse-when-single
  (let [result (first (lang/meme->forms "when(>(x 0) println(\"positive\"))"))]
    (is (= 'when (first result)))
    (is (= '(> x 0) (second result)))
    (is (= '(println "positive") (nth result 2)))))

(deftest parse-cond
  (let [result (first (lang/meme->forms "cond(\n  >(x 0)   \"positive\"\n  ==(x 0)  \"zero\"\n  :else    \"negative\")"))]
...
(deftest parse-try-catch
  (let [result (first (lang/meme->forms "try(dangerous-operation() catch(Exception e log/error(\"Failed:\" e) default-value))"))]
    (is (= 'try (first result)))
    (let [parts (rest result)]
      (is (some #(and (seq? %) (= 'catch (first %))) parts)))))

(deftest parse-try-catch-finally
  (let [result (first (lang/meme->forms "try(dangerous() catch(Exception e handle(e)) finally(cleanup()))"))]
    (is (= 'try (first result)))
    (let [parts (rest result)]
      (is (some #(and (seq? %) (= 'catch (first %))) parts))
...
(deftest parse-try-finally-only
  (let [result (first (lang/meme->forms "try(dangerous() finally(cleanup()))"))]
    (is (= 'try (first result)))
    (let [parts (rest result)]
      (is (some #(and (seq? %) (= 'finally (first %))) parts)))))
...
(deftest parse-thread-first
  (let [result (first (lang/meme->forms "->(account update(:balance *(1.05)) assoc(:status :processed))"))]
    (is (= '-> (first result)))
    (is (= 'account (second result)))))

(deftest parse-thread-last
  (let [result (first (lang/meme->forms "->>(accounts filter(:active) map(:balance) reduce(+))"))]
    (is (= '->> (first result)))
    (is (= 'accounts (second result)))))

(deftest parse-thread-last-multiline
  (let [result (first (lang/meme->forms "->>(accounts\n  filter(:active)\n  map(fn([a] update(a :balance *(:balance(a) 1.05))))\n  remove(fn([a] neg?(:balance(a))))\n  reduce(+ 0))"))]
    (is (= '->> (first result)))
    (is (= 6 (count result)))))
...
(deftest parse-ns-basic
  (let [result (first (lang/meme->forms "ns(my.accounts :require([clojure.string :as str]))"))]
    (is (= 'ns (first result)))
...
(deftest parse-ns-with-import
  (let [result (first (lang/meme->forms "ns(my.accounts :require([clojure.string :as str] [clojure.set :as set]) :import([java.util Date UUID]))"))]
    (is (= 'ns (first result)))
    (is (= 4 (count result)))
...
(deftest parse-ns-with-docstring
  (let [result (first (lang/meme->forms "ns(my.accounts \"Account management.\" :require([clojure.string :as str]))"))]
    (is (= 'ns (first result)))
    (is (= 'my.accounts (second result)))
...
(deftest parse-ns-with-refer
  (let [result (first (lang/meme->forms "ns(my.accounts :require([my.db :refer [query connect]]))"))]
    (is (= 'ns (first result)))
    (let [require-clause (nth result 2)
...
(deftest parse-defprotocol
  (let [result (first (lang/meme->forms "defprotocol(Drawable draw([this canvas]) bounds([this]))"))]
...
(deftest parse-defrecord
  (let [result (first (lang/meme->forms "defrecord(Circle [center radius])"))]
...
(deftest parse-defmulti
  (let [result (first (lang/meme->forms "defmulti(area :shape)"))]
    (is (= 'defmulti (first result)))
    (is (= 'area (second result)))
    (is (= :shape (nth result 2)))))

(deftest parse-defmethod
  (let [result (first (lang/meme->forms "defmethod(area :circle [{:keys [radius]}] *(Math/PI radius radius))"))]
...
(deftest parse-atom-def
  (is (= '[(def state (atom {:count 0}))]
         (lang/meme->forms "def(state atom({:count 0}))"))))

(deftest parse-swap
  (let [result (first (lang/meme->forms "swap!(state update(:count inc))"))]
...
(deftest everything-is-a-call
  (testing "def is a call"
    (is (= '[(def x 42)] (lang/meme->forms "def(x 42)"))))
  (testing "defn is a call"
    (is (= '[(defn f [x] (+ x 1))] (lang/meme->forms "defn(f [x] +(x 1))"))))
  (testing "let is a call"
    (is (= '[(let [x 1] (+ x 2))] (lang/meme->forms "let([x 1] +(x 2))"))))
  (testing "if is a call"
    (is (= '[(if true 1 2)] (lang/meme->forms "if(true 1 2)"))))
  (testing "when is a call"
    (is (= '[(when true (println "yes"))] (lang/meme->forms "when(true println(\"yes\"))"))))
  (testing "try/catch/finally are calls"
    (is (= '[(try (risky) (catch Exception e (handle e)) (finally (cleanup)))]
           (lang/meme->forms "try(risky() catch(Exception e handle(e)) finally(cleanup()))"))))
  (testing "do is a call"
    (is (= '[(do (a) (b))] (lang/meme->forms "do(a() b())"))))
  (testing "ns is a call"
    (is (= '[(ns foo (:require [bar]))] (lang/meme->forms "ns(foo :require([bar]))"))))
  (testing "defmulti is a call"
    (is (= '[(defmulti area :shape)] (lang/meme->forms "defmulti(area :shape)"))))
  (testing "defmethod is a call"
    (is (= '[(defmethod area :circle [this] (:radius this))]
           (lang/meme->forms "defmethod(area :circle [this] :radius(this))"))))
  (testing "defprotocol is a call"
    (is (= '[(defprotocol Drawable (draw [this canvas]))]
           (lang/meme->forms "defprotocol(Drawable draw([this canvas]))"))))
  (testing "defrecord is a call"
    (is (= '[(defrecord Foo [x y])]
           (lang/meme->forms "defrecord(Foo [x y])")))))
...
(deftest parse-do-as-symbol
  (is (= '[do] (lang/meme->forms "do"))))

(deftest parse-catch-as-symbol
  (is (= '[catch] (lang/meme->forms "catch"))))

(deftest parse-finally-as-symbol
  (is (= '[finally] (lang/meme->forms "finally"))))
...
(deftest parse-for-while
  (testing "for with :while modifier"
    (let [result (first (lang/meme->forms "for([x xs, :while <(x 10)] x)"))
...
(deftest parse-for-let
  (testing "for with :let modifier"
    (let [result (first (lang/meme->forms "for([x [1 2 3], :let [y *(x 10)]] +(x y))"))
          bindings (second result)]
      (is (= 'for (first result)))
