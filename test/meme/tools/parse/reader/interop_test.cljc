(ns meme.tools.parse.reader.interop-test
...
(deftest parse-double-dot-interop
  (let [result (first (lang/meme->forms "..(obj method1() method2())"))]
    (is (= '.. (first result)))))

(deftest parse-doto-interop
  (let [result (first (lang/meme->forms "doto(StringBuilder.() .append(\"a\") .append(\"b\"))"))]
    (is (= 'doto (first result)))
    (is (= 4 (count result)))))

(deftest parse-method-on-expression
  (let [result (first (lang/meme->forms ".trim(.concat(\"a\" \"b\"))"))]
    (is (= '.trim (first result)))
    (is (seq? (second result)))
    (is (= '.concat (first (second result))))))

(deftest parse-static-field-standalone
  (is (= 'Integer/MAX_VALUE (first (lang/meme->forms "Integer/MAX_VALUE")))))

(deftest parse-constructor-in-binding
  (let [result (first (lang/meme->forms "let([sb StringBuilder.()] sb)"))]
    (is (= 'let (first result)))
