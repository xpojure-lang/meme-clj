(ns meme.tools.parse.expander-test
  "Unit tests for meme.tools.parse.expander: syntax-quote expansion and
   MemeRaw unwrapping."
...
(deftest expand-syntax-quote-with-resolve-symbol
  ...
  #?(:clj
     (testing "nested syntax-quote with resolver matches Clojure behavior"
       ...
       (require 'meme.tools.run)
       (let [resolver (resolve 'meme.tools.run/default-resolve-symbol)
             expanded (first (expander/expand-forms
                               (lang/meme->forms "``x")
                               {:resolve-symbol resolver}))
