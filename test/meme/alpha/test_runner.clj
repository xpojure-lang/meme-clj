...
(defn run-eval-tests
  "Run all .meme files in dir via eval. Each file asserts its own correctness."
  [dir]
...
;; ---------------------------------------------------------------------------
;; Fixture-based tests (test/examples/fixtures/*.meme + *.edn)
;; ---------------------------------------------------------------------------

(defn run-fixture-tests
  "Run fixture tests: parse .meme, compare to .edn expected forms."
  [dir]
  (let [file-obj (io/file dir)
        files (sort (or (.listFiles file-obj) []))
        meme-files (filter #(.endsWith (.getName %) ".meme") files)]
