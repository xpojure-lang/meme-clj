(ns m1clj-lang.reader.data-literals-test
...
(deftest parse-map-literal
  (let [result (first (lang/m1clj->forms "{:name \"Andriy\" :age 45}"))]
    (is (map? result))
    (is (= "Andriy" (:name result)))))

(deftest parse-set-literal
  (is (= #{1 2 3} (first (lang/m1clj->forms "#{1 2 3}")))))

(deftest parse-nested-data
  (let [result (first (lang/m1clj->forms "{:users [{:name \"a\"} {:name \"b\"}]}"))]
...
(deftest parse-nested-maps
  (let [result (first (lang/m1clj->forms "{:a {:b {:c 1}}}"))]
    (is (= 1 (get-in result [:a :b :c])))))

(deftest parse-numeric-keys-in-maps
  (let [result (first (lang/m1clj->forms "{1 \"one\" 2 \"two\"}"))]
    (is (map? result))
    (is (= "one" (get result 1)))))

(deftest parse-keyword-as-function
  (let [result (first (lang/m1clj->forms ":name({:name \"foo\"})"))]
    (is (= :name (first result)))
    (is (map? (second result)))))

(deftest parse-comma-as-whitespace
  (is (= '[[1 2 3]] (lang/m1clj->forms "[1, 2, 3]")))
  (let [result (first (lang/m1clj->forms "{:a 1, :b 2}"))]
    (is (= 1 (:a result)))
    (is (= 2 (:b result)))))

(deftest parse-deeply-nested-vectors
  (is (= [[[1]]] (first (lang/m1clj->forms "[[[1]]]")))))

(deftest parse-mixed-collection-nesting
  (let [result (first (lang/m1clj->forms "[{:a #{1 2}} {:b [3 4]}]"))]
    (is (vector? result))
    (is (= 2 (count result)))
