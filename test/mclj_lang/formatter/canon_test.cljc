...
;; ---------------------------------------------------------------------------
;; Comments from :mclj/leading-trivia metadata
;; ---------------------------------------------------------------------------

(deftest comment-before-form
  (let [form (with-meta '(foo x) {:mclj/leading-trivia "; a comment\n"})
        result (fmt-canon/format-form form)]
    (is (re-find #"^; a comment\n" result))
    (is (re-find #"foo\(x\)" result))))

(deftest multiple-comment-lines
  (let [form (with-meta '(foo) {:mclj/leading-trivia "; line 1\n; line 2\n"})
        result (fmt-canon/format-form form)]
    (is (re-find #"; line 1\n; line 2\n" result))))

(deftest no-ws-metadata
  (testing "form without :mclj/leading-trivia has no comment prefix"
    (is (= "+(1 2)" (fmt-canon/format-form '(+ 1 2))))))
...
;; NOTE: The experimental pipeline's CST reader attaches trivia (:mclj/leading-trivia) from
