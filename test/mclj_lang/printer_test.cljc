...
;; ---------------------------------------------------------------------------
;; Reader-sugar metadata — :mclj/sugar tags
;; ---------------------------------------------------------------------------

(deftest quote-sugar-on-vs-off
  (testing "(quote x) WITH :mclj/sugar renders as 'x"
    (let [form (with-meta (list 'quote 'x) {:mclj/sugar true})]
...
(deftest deref-sugar-on-vs-off
  (testing "(clojure.core/deref x) WITH sugar renders as @x"
    (let [form (with-meta (list 'clojure.core/deref 'x) {:mclj/sugar true})]
      (is (= "@x" (render-at form ##Inf)))))
...
(deftest var-sugar-on-vs-off
  (testing "(var x) WITH sugar renders as #'x"
    (let [form (with-meta (list 'var 'x) {:mclj/sugar true})]
      (is (= "#'x" (render-at form ##Inf)))))
...
(deftest user-metadata-renders-as-caret
  ...
  (testing "^{:a 1 :b 2} with an explicit map renders the map"
    ...
    (let [form (with-meta 'x {:a 1 :b 2 :mclj/meta-chain [{:a 1 :b 2}]})
...
(deftest internal-metadata-does-not-leak
  (testing "internal keys (:line, :col, :mclj/leading-trivia) are not emitted"
    (let [form (with-meta 'x {:line 1 :col 1 :mclj/leading-trivia " "})]
...
(deftest anon-fn-shorthand-with-sugar-tag
  (testing "(fn [%1] (body)) WITH :mclj/sugar renders as #(...)"
    (let [form (with-meta (list 'fn '[%1] (list 'inc '%1)) {:mclj/sugar true})]
...
(deftest anon-fn-sugar-only-triggers-for-percent-params
  (testing "sugar tag on non-% params must NOT produce #() — that changes semantics"
    ...
    (let [form (with-meta (list 'fn '[x] (list 'inc 'x)) {:mclj/sugar true})
          out (render-at form ##Inf)]
