...
Everything else (data literals, reader syntax, destructuring, commas-as-whitespace) is unchanged from Clojure. The CLI is self-hosted in `.mclj`. Programs run on Babashka, Clojure JVM, or ClojureScript without modification.
...
# .mclj example tests (Babashka) — runs test/examples/tests/*.mclj
bb test-meme
...
# Run a .mclj file
bb meme run file.mclj

# Start meme REPL
bb meme repl

# Convert between meme and Clojure
bb meme to-clj file.mclj     # .mclj → Clojure
bb meme to-mclj file.clj     # .clj → meme

# Format .mclj files (normalize syntax via canonical formatter)
bb meme format file.mclj     # in-place
bb meme format src/          # directory, recursive
bb meme format file.mclj --stdout  # print to stdout

# Compile .mclj to .clj (for classpath use without runtime patching)
bb meme transpile src/            # output to target/mclj (default; alias: `compile`)
...
- **`mclj-lang.*`** — Meme language implementation. The *syntactic* surface lives here (grammar, parselets, printer, form-shape, formatters). Infrastructure files like `mclj-lang.lexlets`, `mclj-lang.run`, `mclj-lang.repl` are thin shims that inject meme's grammar/banner and delegate to `meme.tools.clj.*`.
...
This is a "kinds + infrastructure" layout, not a strict top-down tiered architecture. `mclj-lang.api` requiring `meme.registry` (for self-registration) and `mclj-lang.run` requiring `meme.loader` (for auto-install) are intentional — the infrastructure is shared. The real layering rule is: **`mclj-lang.*` must not require `meme.cli`**, and **`meme.tools.{parser,lexer,render}` plus `meme.tools.clj.*` must not require anything from `mclj-lang.*` or `meme.*`** — with one carve-out: `meme.tools.clj.run` and `meme.tools.clj.repl` depend on `meme.loader` so embedders get `.mclj` `require`/`load-file` for free. That coupling is intentional and narrow (one `install!` call each); no other `meme.tools.*` ns reaches into `meme.*`.
...
The pipeline is lossless (CST preserves all tokens including delimiters and trivia) and data-driven (the Pratt parser is generic — meme syntax is defined by a grammar spec in `mclj-lang.grammar`).

The generic parser engine, scanlet builders, and render engine live in `meme.tools.*` — they are language-agnostic and reusable. Meme-specific grammar, scanlets (lexlets), parselets, and CST reader live in `mclj-lang.*`.
...
1. **Notation** (`mclj-lang.printer`) — how a call renders (parens, delimiter placement, `:mclj` vs `:clj` mode). Knows nothing about form names or slot semantics beyond the fallback recursion.
2. **Form-shape** (`mclj-lang.form-shape`) — what the parts of a special form *mean*. A registry maps head symbols to decomposers; each decomposer produces `[[slot-name value] ...]`. Lang-owned: each lang carries its own registry. See `doc/form-shape.md` for the slot vocabulary.
3. **Style** (`mclj-lang.formatter.canon/style` and alternatives) — opinions *per slot name*, not per form. `:head-line-slots` keeps named slots with the call head on break; `:force-open-space-for` controls the `head( ` convention; `:slot-renderers` overrides the printer defaults for `:bindings`/`:clause`/custom slots.
...
- `meme.tools.clj.stages` (.cljc) — Composable pipeline stages: `step-parse`, `step-read`, `step-evaluate-reader-conditionals`, `step-expand-syntax-quotes`. Each is `ctx → ctx`. `run` composes parse+read (tooling pipeline). `stage-contracts` (public data) declares required ctx keys and opts keys per stage; each stage validates its ctx against the contract at entry and throws `:mclj/pipeline-error` on miscomposition via an internal `check-contract!`. `step-parse` requires `:grammar` in opts (no implicit default). Portable.
...
**Meme language** (`mclj-lang.*`) — meme-specific implementation:

- `mclj-lang.api` (.cljc) — Public API and lang composition. Provides `mclj->forms`, `forms->mclj`, `forms->clj`, `clj->forms`, `mclj->clj`, `clj->mclj`, `format-mclj-forms`. Also lang commands: `format-mclj`, `to-clj`, `to-mclj`, `lang-map`. Injects meme's grammar into the commons pipeline. `clj->forms` and `clj->mclj` are JVM only. Portable.
- `mclj-lang.grammar` (.cljc) — Meme language grammar spec: maps characters to scanlets and parselets. The complete syntactic specification of M-expression syntax as data. Portable.
- `mclj-lang.parselets` (.cljc) — Meme-specific compound parselets: call adjacency detection, `#` dispatch sub-routing, tilde (`~`/`~@`), and the M-expression call rule. Portable.
- `mclj-lang.lexlets` (.cljc) — Thin shim forwarding to `meme.tools.clj.lex`. Meme inherits all Clojure lexical conventions; this namespace keeps mclj-lang's lexical identity and is the place where any future meme-specific lexical rule would live. Portable.
- `mclj-lang.form-shape` (.cljc) — Semantic decomposition of special forms into named slots (`:name`, `:params`, `:bindings`, `:clause`, `:body`, etc.). The middle layer between notation (printer) and style (formatter). Owns the stable slot vocabulary. See `doc/form-shape.md`. Portable.
- `mclj-lang.printer` (.cljc) — Wadler-Lindig Doc tree builder: `to-doc` (form → Doc tree). Single source of truth for meme and Clojure output modes. Delegates layout to `meme.tools.render`. Dispatches on form-shape slots and applies style's slot-keyed opinions. Portable.
- `mclj-lang.formatter.flat` (.cljc) — Flat formatter: composes printer + render at infinite width. `format-form`, `format-forms`, `format-clj`. Single-line output. Portable.
- `mclj-lang.formatter.canon` (.cljc) — Canonical formatter: composes printer + render at target width. `format-form`, `format-forms`. Width-aware multi-line output. Used by `meme format` CLI. Portable.
- `mclj-lang.run` (.clj) — Thin shim: injects meme's grammar and delegates to `meme.tools.clj.run`. Re-exports `default-resolve-symbol` for backwards compat. JVM/Babashka only.
- `mclj-lang.repl` (.clj) — Thin shim: injects meme's grammar and banner and delegates to `meme.tools.clj.repl`. JVM/Babashka only.
...
- `meme.loader` (.clj) — Namespace loader: intercepts `clojure.core/load` (JVM only) and `clojure.core/load-file` (JVM + Babashka) to handle `.mclj` files transparently. Installed automatically by `run-file`, REPL `start`, and the CLI `run` command. `require` finds `.mclj` namespaces on the classpath (JVM only — Babashka's SCI bypasses `clojure.core/load`). `load-file` handles `.mclj` files by filesystem path on both platforms. `.mclj` takes precedence when both `.mclj` and `.clj` exist. `install!`/`uninstall!` for manual control. JVM/Babashka only.

**App tier** (`meme.*`):

- `meme.cli` (.clj) — Unified CLI: `run`, `repl`, `to-clj`, `to-mclj`, `format`, `transpile` (alias: `compile`), `build`, `inspect`, `version`. Generic dispatcher — commands delegate to lang map functions. Babashka entry point via `bb.edn`.
...
| Core translation | mclj-lang.{api, grammar, parselets, lexlets, form-shape, printer, formatter.flat, formatter.canon} | JVM, Babashka, ClojureScript |
| Runtime infra | meme.tools.{run, repl}, meme.tools.clj.{run, repl}, mclj-lang.{run, repl}, meme.{registry, loader} | JVM, Babashka |
...
- `doc/language-reference.md` — Complete syntax reference for writing .mclj code.
...
- `.mclj` example files in `test/examples/tests/` are eval-based (self-asserting). Numeric prefixes (`01_`, `02_`, ...) control execution order — the test runner sorts alphabetically, so fundamentals (core rules, definitions) run before features that build on them. New files should continue the numbering sequence.
...
Tests are split across `test/mclj_lang/` (language-specific) and `test/meme/` (infrastructure, integration, regression).

| File | What belongs here |
|------|-------------------|
| `mclj_lang/stages_test` | Pipeline stages: parse → read, syntax-quote expansion |
| `mclj_lang/reader/call_syntax_test` | M-expression call syntax: head type x spacing x arity matrix |
| `mclj_lang/reader/calls_test` | All Clojure forms as calls: def, defn, fn, let, loop, for, if, when, cond, try, threading, ns, protocols, records, multimethods, concurrency, "everything is a call" |
| `mclj_lang/reader/interop_test` | Java interop: .method, Class/static, .-field, constructors |
| `mclj_lang/reader/data_literals_test` | Data literal passthrough: vectors, maps, sets, keywords, numbers |
| `mclj_lang/reader/dispatch_test` | Reader macros and dispatch: @, ^, ', #', #_, #(), regex, char, tagged literals, reader conditionals, namespaced maps |
| `mclj_lang/reader/errors_test` | Error cases, rejected forms (unquote outside backtick), error messages with locations, CLJS-specific errors |
...
| `mclj_lang/form_shape_test` | Form-shape decomposition: per-form decomposer output, structural fallback, registry extension |
| `mclj_lang/printer_test` | Printer-level seams: `:slot-renderers` override, default slot renderers, unknown-slot fallback |
| `mclj_lang/formatter/flat_test` | Flat formatter: single-line meme/clj output, reader sugar, individual form cases |
| `mclj_lang/formatter/canon_test` | Canonical formatter: width-aware formatting, multi-line layout, comments |
...
| `mclj_lang/api_test` | Language API (`mclj->forms`, `forms->mclj`, `format-mclj-forms`, etc.) |
...
| `mclj_lang/repl_test` | REPL infrastructure (`input-state`, `read-input`). JVM only. |
| `mclj_lang/run_test` | File runner: `run-string`, `run-file`, shebang handling, custom eval-fn |
| `meme/loader_test` | Namespace loader: `load` interception, classpath `.mclj` discovery, `install!`/`uninstall!` lifecycle. JVM/Babashka. |
...
- **Diagnostics (clj-kondo)**: Catches unused requires, unresolved symbols, unused bindings. Known noise to ignore:
  ...
  - `.cljc` files with `#?` reader conditionals — clj-kondo analyzes one platform branch and flags requires/vars used only in the other branch as unused. Affects `meme/tools/clj/resolve.cljc`, `mclj_lang/repl_test.cljc`, `meme/tools/clj/errors_test.cljc`, `meme/regression/errors_test.cljc`, `mclj_lang/reader/dispatch_test.cljc`, `meme/regression/scan_test.cljc`.
...
## meme Syntax Quick Reference (for writing .mclj code)
