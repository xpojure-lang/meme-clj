(ns m2clj-lang.printer
  "m2clj printer: Clj* AST or plain Clojure forms → Doc trees.
   Builds Wadler-Lindig Doc trees handling m2clj syntax (call notation,
   bare-paren list literals, sugar, metadata, comments) and Clojure
   output mode.
   ...
   `format-m2clj` reconstructs the user's original syntax. Plain forms
   (the `forms->m2clj` / `forms->clj` path) are printed structurally —
   ...
   In `:m2clj` mode the printer canonicalises CljQuote nodes whose form
   is a non-empty list to bare-paren rendering `(x y z)` regardless of
   the source's :notation field — `'(x y z)` in m2clj reads as a
   double-quote, so the canonical form is bare paren.
...
;; semantic slot names (emitted by `m2clj-lang.form-shape/decompose`), not
...
(def default-slot-renderers
  "Default per-slot structural renderers, keyed by slot name (see
   `m2clj-lang.form-shape` for the vocabulary).  Each renderer is
...
(defn- call-doc
  "Build Doc for a call form.
   ...
   in both `:m2clj` and `:clj` mode and the resulting head/body slot
   split is the same. Only the *geometry* differs by mode — m2clj writes
   ...
   key `:force-open-space-for` is m2clj-specific and is ignored under
   `:clj` mode."
  [head args ctx]
  (let [mode     (:mode ctx)
...
(defn- to-doc-ast-node
  ...
  (let [mode (:mode ctx)]
    (cond
      ...
      ;; m2clj canonicalises CljQuote{form: non-empty CljList} to bare-paren
      ;; rendering regardless of the source's :notation field. The user-written
      ;; `'(x y z)` reads to (quote (quote (x y z))) — a double-quote that's
      ;; almost certainly unintended; the canonical form is bare paren `(x y z)`.
      ;; Empty list inside a quote stays `'()` because m2clj's `()` is the empty
      ;; list (no quote applied).
      (let [inner (:form node)
            list-quote? (and (= :m2clj (:mode ctx))
                             (instance? CljList inner)
                             (seq (effective-ast-children (:children inner))))]
        (if list-quote?
          (collection-doc "(" ")" (effective-ast-children (:children inner)) ctx true)
          (render/doc-cat doc-quote (to-doc-inner inner ctx))))
      ...
      (let [body (:body node)]
        (cond
          ...
          ;; :m2clj mode: keep #(body) wrapping intact.
          :else
...
(defn- to-doc-form
  ...
  hash order. ctx is {:mode :m2clj|:clj, :style style-map-or-nil}."
  [form ctx]
  (or (to-doc-ast-node form ctx)
      (cond
        ...
        ;; (quote x) → notation depends on mode:
        ;;   :m2clj + non-empty sequential x → bare-paren `(elements...)`
        ;;   :m2clj + anything else          → tick `'x`
        ;;   :clj   + anything               → tick `'x`  (Clojure idiom)
        ;; Routes around the general call-form path, which would emit
        ;; `quote(x)` (m2clj) or `(quote x)` (clj) and round-trip correctly
        ;; but un-idiomatically.
        (and (#{:m2clj :clj} (:mode ctx))
             (seq? form) (= 'quote (first form))
             (= 2 (count form)))
        (let [arg (second form)]
          (if (and (= :m2clj (:mode ctx)) (sequential? arg) (seq arg))
            (collection-doc "(" ")" (vec arg) ctx true)
            (render/doc-cat doc-quote (to-doc-inner arg ctx))))
