(ns m1clj-lang.run-test
  "Tests for the m1clj-lang.run module."
...
(deftest run-file-basic
  (testing "runs a .m1clj file and returns last result"
    (let [tmp (java.io.File/createTempFile "m1clj-test" ".m1clj")]
      (try
        (spit tmp "def(x 42)\nx")
...
(deftest run-file-not-found
  (testing "non-existent file throws"
    (is (thrown? Exception (run/run-file "/tmp/nonexistent-m1clj-file-12345.m1clj")))))
...
(deftest run-file-syntax-quote-resolves-symbols
  ...
  (testing "`map in a file resolves to clojure.core/map (matches run-string)"
    (let [tmp (java.io.File/createTempFile "m1clj-resolve-test" ".m1clj")]
      (try
        (spit tmp "`map")
...
;; m1clj-lang.run concern (see run-file's :resolve-lang-for-path opt).
