(ns m1clj-lang.repl-test
  "Tests for m1clj-lang.repl input-state logic and read-input multi-line accumulation.
...
(def ^:private input-state m1clj-lang.repl/input-state)
(def ^:private read-input* @#'meme.tools.repl/read-input)
(defn- stages-with-m1clj-grammar
...
(defn- read-input
  ...
  (read-input* prompt read-line-fn reader-opts stages-with-m1clj-grammar))
...
(deftest incomplete-errors-carry-ex-data-key
  (testing "EOF errors have :incomplete true in ex-data"
    (doseq [input ["+(1 2" "[1 2 3" "{:a 1"]]
      (let [e (try (lang/m1clj->forms input)
                   nil
                   (catch Exception e e))]
...
(deftest read-input-malformed-returns-immediately
  (testing "malformed input returns immediately with :error"
    (let [result (binding [*out* (java.io.StringWriter.)]
...
;; named ex-info, not silently boot a half-configured REPL. Built-in m1clj-lang
...
(deftest tools-clj-repl-start-without-grammar-throws
  (testing "missing :grammar surfaces as :m1clj/missing-grammar ex-info"
    (try (clj-repl/start {:install-loader? false})
         (is false "expected :m1clj/missing-grammar but no exception thrown")
         (catch clojure.lang.ExceptionInfo e
           (is (= :m1clj/missing-grammar (:type (ex-data e))))
