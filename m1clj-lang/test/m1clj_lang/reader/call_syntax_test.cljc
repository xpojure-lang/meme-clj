(ns m1clj-lang.reader.call-syntax-test
...
(deftest symbol-head-arity-0
  (is (= '[(foo)] (lang/m1clj->forms "foo()"))))

(deftest symbol-head-arity-1
  (is (= '[(foo x)] (lang/m1clj->forms "foo(x)"))))

(deftest symbol-head-arity-2
  (is (= '[(foo x y)] (lang/m1clj->forms "foo(x y)"))))

(deftest symbol-head-arity-3+
  (is (= '[(foo a b c d)] (lang/m1clj->forms "foo(a b c d)"))))

(deftest symbol-head-operator
  (is (= '[(+ 1 2 3)] (lang/m1clj->forms "+(1 2 3)"))))

(deftest symbol-head-nested
  (is (= '[(a (b (c x)))] (lang/m1clj->forms "a(b(c(x)))"))))
...
(deftest keyword-head-arity-0
  (is (= '(:foo) (first (lang/m1clj->forms ":foo()")))))

(deftest keyword-head-arity-1
  (is (= '(:foo x) (first (lang/m1clj->forms ":foo(x)")))))

(deftest keyword-head-arity-2
  (is (= '(:foo x y) (first (lang/m1clj->forms ":foo(x y)")))))

(deftest keyword-head-arity-3+
  (is (= '(:foo a b c d) (first (lang/m1clj->forms ":foo(a b c d)")))))

(deftest keyword-head-namespaced
  (is (= '(:ns/key x) (first (lang/m1clj->forms ":ns/key(x)")))))

(deftest keyword-head-require-pattern
  (is (= '(:require [bar]) (first (lang/m1clj->forms ":require([bar])")))))

(deftest keyword-head-import-pattern
  (is (= '(:import [java.util Date]) (first (lang/m1clj->forms ":import([java.util Date])")))))
...
(deftest vector-head-arity-0
  (is (= '([x]) (first (lang/m1clj->forms "[x]()")))))

(deftest vector-head-arity-1
  (is (= '([x] 1) (first (lang/m1clj->forms "[x](1)")))))

(deftest vector-head-arity-2
  (is (= '([x] (+ x 1) x) (first (lang/m1clj->forms "[x](+(x 1) x)")))))

(deftest vector-head-multi-params
  (is (= '([x y] (+ x y)) (first (lang/m1clj->forms "[x y](+(x y))")))))

(deftest vector-head-empty-vector
  (is (= '([] 42) (first (lang/m1clj->forms "[](42)")))))
...
(deftest set-head-arity-1
  (let [form (first (lang/m1clj->forms "#{:a :b}(x)"))]
    (is (set? (first form)))
    (is (= 'x (second form)))))

(deftest set-head-arity-0
  (let [form (first (lang/m1clj->forms "#{:a}()"))]
    (is (set? (first form)))
    (is (= 1 (count form)))))
...
(deftest map-head-arity-1
  (let [form (first (lang/m1clj->forms "{:a 1}(:a)"))]
    (is (map? (first form)))
    (is (= :a (second form)))))

(deftest map-head-arity-0
  (let [form (first (lang/m1clj->forms "{:a 1}()"))]
    (is (map? (first form)))
    (is (= 1 (count form)))))
...
(deftest set-spacing-adjacent
  (let [form (first (lang/m1clj->forms "#{:a}(x)"))]
    (is (= 'x (second form)))))
...
(deftest pattern-defn-single-arity
  (is (= '[(defn greet [name] (str "Hello " name))]
         (lang/m1clj->forms "defn(greet [name] str(\"Hello \" name))"))))

(deftest pattern-defn-multi-arity
  (let [result (first (lang/m1clj->forms "defn(foo [x](x) [x y](+(x y)))"))]
...
(deftest pattern-ns-require
  (let [result (first (lang/m1clj->forms "ns(my.app :require([clojure.string :as str]))"))]
    (is (= 'ns (first result)))
    (is (= 'my.app (second result)))
    (is (= :require (first (nth result 2))))))

(deftest pattern-ns-require-import
  (let [result (first (lang/m1clj->forms "ns(my.app :require([clojure.string :as str]) :import([java.util Date]))"))]
    (is (= 4 (count result)))
    (is (= :require (first (nth result 2))))
...
(deftest pattern-defrecord-with-impls
  (let [result (first (lang/m1clj->forms "defrecord(Circle [r] Shape area([this] *(Math/PI r r)))"))]
...
(deftest pattern-let-with-calls
  (is (= '[(let [x (+ 1 2)] (* x x))]
         (lang/m1clj->forms "let([x +(1 2)] *(x x))"))))

(deftest pattern-threading
  (is (= '[(-> x (foo 1) (bar 2))]
         (lang/m1clj->forms "->(x foo(1) bar(2))"))))

(deftest pattern-try-catch
  (is (= '[(try (risky) (catch Exception e (handle e)))]
         (lang/m1clj->forms "try(risky() catch(Exception e handle(e)))"))))
