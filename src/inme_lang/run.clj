(ns inme-lang.run
  "Inme-specific eval pipeline. Delegates to `meme-lang.run` with inme's
...
(defn run-string
  "Read inme source string, eval each form, return the last result."
  ([s] (run-string s {}))
  ([s eval-fn-or-opts]
   {:pre [(string? s)]}
...
(defn run-file
  "Read and eval an inme file. Returns the last result."
  ([path] (run-file path {}))
