...
;; ---------------------------------------------------------------------------
;; Eval-based tests (examples/tests/*.beme — self-asserting)
;; ---------------------------------------------------------------------------
...
;; ---------------------------------------------------------------------------
;; Fixture-based tests (examples/fixtures/*.beme + *.edn)
;; ---------------------------------------------------------------------------
                        ...
                        (let [base (str/replace (.getName f) #"\.beme$" "")
                          ...
                          (if-not (.exists edn-file)
                            (do (println "SKIP (no .edn fixture)")
                                :skip)
                            (let [beme-src (try (slurp (str f)) (catch Exception _ nil))]
                              (try
...
(defn run-all-beme-tests
  ...
  []
  (println "Eval tests (examples/tests):")
  (let [eval-fails (run-eval-tests "examples/tests")]
    (println)
    (println "Fixture tests (examples/fixtures):")
    (let [fixture-fails (run-fixture-tests "examples/fixtures")
