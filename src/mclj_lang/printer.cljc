...
;; ---------------------------------------------------------------------------
;; Comment extraction from :mclj/leading-trivia metadata
;; ---------------------------------------------------------------------------

(defn- extract-comments
  "Extract comment lines from a :mclj/leading-trivia metadata string.
...
(defn- form-comments
  "Get comment lines from a form's :mclj/leading-trivia metadata, or nil."
  [form]
  (when (and (some? form)
             #?(:clj  (instance? clojure.lang.IMeta form)
                :cljs (satisfies? IMeta form))
             (meta form))
    (extract-comments (:mclj/leading-trivia (meta form)))))
...
(defn- anon-fn-shorthand?
  "Can (fn [params] body) be printed as #(body)?
   Only when :mclj/sugar tagged by reader AND params are %-style."
  [form]
  (and (:mclj/sugar (meta form))
       (seq? form)
       (= 'fn (first form))
       (= 3 (count form))
       (vector? (second form))
       ;; RT6-F21: verify params are %-style — programmatic misuse of
       ;; :mclj/sugar on non-% fns would silently change semantics
...
(defn- to-doc-form
  ...
  (let [mode (:mode ctx)]
    (cond
      ...
      (let [chain (:mclj/meta-chain (meta form))
      ...
      ;; F7: when :mclj/bare-percent, restore % from %1 in body before printing.
      (and (anon-fn-shorthand? form)
           (or (not= mode :clj) (seq? (nth form 2))))
      (let [raw-body (nth form 2)
            body (if (:mclj/bare-percent (meta form))
      ...
      (let [head (first form)]
        (cond
          ;; @deref sugar
          (and (= head 'clojure.core/deref) (:mclj/sugar (meta form)))
          (render/doc-cat doc-at (to-doc-inner (second form) ctx))

          ;; 'quote sugar
          (and (= head 'quote) (:mclj/sugar (meta form)))
          (render/doc-cat doc-quote (to-doc-inner (second form) ctx))

          ;; #'var sugar
          (and (= head 'var) (:mclj/sugar (meta form)))
      ...
      (let [inner (:form form)
            ...
            inner (if (and (seq? inner)
                           ...
                           (:mclj/sugar (meta inner)))
                    (with-meta inner (dissoc (meta inner) :mclj/sugar))
                    inner)]
      ...
      ;; Map — reconstruct #:ns{} or #::alias{} when :mclj/namespace-prefix metadata present
      (map? form)
      (if-let [ns-str (:mclj/namespace-prefix (meta form))]
      ...
      ;; Set — use :mclj/insertion-order for insertion-order output, validated against actual contents
      (set? form)
      (let [order (:mclj/insertion-order (meta form))
            ;; Use :mclj/insertion-order when it matches set size (not stale), otherwise fall back
